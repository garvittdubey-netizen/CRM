...
**Phase 15.1: Landing-page polish (logo backdrop chip, dark theme toggle, real WhatsApp brand mark, new MITCS logo)** — COMPLETE (2026-05-22)
  ...
  - **Logo visibility fix (light backgrounds)**: The BuilderOne logo was authored against a black canvas (gold + white wordmark). On the light landing page header / footer / login page / sidebar, the white wordmark portion was effectively invisible against white. Fix: wrapped every BuilderOne logo instance in a slate-900 (navy-black) rounded chip with a thin 1px ring — preserves the original logo art, restores readability on light surfaces, and looks intentional on dark surfaces too. Applied at:
    - Landing-page top nav (new internal `<LogoChip size="sm" />` component inside `LandingPage.tsx`).
    - Landing-page footer (same chip, slate-900 sits naturally on the dark footer).
    - Login page logo header (`pages/LoginPage.tsx`) — chip wraps the existing `<img>`.
    - Sidebar expanded state (`components/layout/Sidebar.tsx`) — chip on both light and dark CRM themes.
    - Sidebar collapsed state — chip wraps the favicon icon mark.
    - Mobile drawer header (`components/layout/MobileSidebar.tsx`) — same chip pattern.
  - **Dark / light theme toggle on the landing page**: Added a sun/moon toggle button in the top nav (`data-testid="theme-toggle-button"`). The toggle flips a local `dark` boolean state, which applies the `dark` class to the landing-page root `<div>` (Tailwind's `darkMode: ['class']` was already on from Phase 14.x — no config change needed). The choice is persisted to `localStorage['landing:theme']` (`'dark' | 'light'`) and falls back to the user's OS `prefers-color-scheme` on first visit.
    - Every section (hero, features, workflow, why, about, CTA, footer, floating WhatsApp tooltip, theme button itself) has matching `dark:` variants:
      * Light: white/slate-50 surfaces, slate-900 text, navy primary buttons, amber accents.
      * Dark: slate-950/900 surfaces, white/slate-100 text, **amber-500 primary buttons** (swapped from navy so they remain visible on the dark background), softer 20% blurred radial gradients.
    - The "About product" card swaps its amber accent for a purple accent in dark mode to harmonise with the new purple MITCS logo (added in this phase, see below).
    - The CRM itself (everything under `/dashboard` and friends) continues to use its own pre-existing `next-themes` toggle inside the Navbar — those two systems are deliberately independent so a visitor toggling the landing-page theme doesn't accidentally change a logged-in user's CRM preference, and vice versa.
  - **Real WhatsApp brand mark**: The generic `MessageCircle` icon from lucide-react was replaced everywhere it represented WhatsApp with a hand-rolled `<WhatsAppIcon />` SVG component that paints the official WhatsApp logo (green `#25D366` chat bubble + white phone receiver glyph). Supports two variants:
    * `variant="brand"` — full-colour brand mark, used on the nav WhatsApp link, the "Contact on WhatsApp" buttons in hero + CTA, the Features grid "WhatsApp Integration" card icon, and the floating action button (which now sits on a white circle so the brand-green chat bubble pops).
    * `variant="inherit"` — single-colour `fill="currentColor"` variant, retained for future text-coloured contexts (none currently used after this phase, kept for follow-up work).
    The floating action button's animate-ping ring was also re-tinted to the WhatsApp brand green (`#25D366`) instead of Tailwind's emerald-500, matching brand guidelines.
  - **MITCS logo replaced**: User uploaded a new MITCS logo (purple hexagons + "Microtechnique IT" wordmark, white background JPEG). It was cleaned via Pillow (white-to-alpha thresholded + tight bbox crop) and saved as transparent PNG to `/app/frontend/public/mitcs-logo.png` (overwriting the prior 4-coloured hexagon mark from Phase 15.0). The About section's logo block was widened to `h-40` and given a white inner card so the purple artwork is readable on BOTH theme variants of the dark "About" card.
  - **Files modified (4)**: `frontend/src/pages/LandingPage.tsx` (full rewrite — adds theme state + WhatsAppIcon component + LogoChip helper + dark: variants everywhere), `frontend/src/pages/LoginPage.tsx` (logo wrapped in dark chip — form logic untouched), `frontend/src/components/layout/Sidebar.tsx` (logo chip on expanded + collapsed states), `frontend/src/components/layout/MobileSidebar.tsx` (logo chip in drawer header).
  - **Files NOT touched**: every backend file, every CRM page, every other component, every shadcn primitive, tailwind config (no new config needed — class-based dark mode was already enabled), `index.css`, `manifest.json`, `index.html`, `App.tsx`, `AuthContext.tsx`, `ProtectedRoute.tsx`, every existing data-testid on pre-existing CRM surfaces, every supervisord program. The Navbar's pre-existing `useTheme()` next-themes integration was deliberately left alone — the landing page's local theme state and the CRM's next-themes state are independent on purpose (see above).
  - **Asset replaced (1)**: `frontend/public/mitcs-logo.png` — purple Microtechnique IT hexagonal mark, 1478×749, white-bg removed via Pillow.
  - **Test-IDs added in this phase (1)**: `theme-toggle-button` (the sun/moon button in the landing-page nav). All Phase-15.0 test-IDs (`landing-page`, `nav-logo-chip`, `hero-*`, `features-grid`, `workflow-steps`, `why-grid`, `mitcs-*`, `cta-*`, `floating-whatsapp-button`, `footer-copy`, `login-logo`, `sidebar-logo`, `sidebar-logo-collapsed`, etc.) preserved verbatim.


**Phase 15.0: BuilderOne CRM rebrand + Landing page + Demo login card** — COMPLETE (2026-05-22)
  - **Scope**: Pure cosmetic / additive layer on top of the existing real-estate CRM. Zero CRM functionality, routes, RBAC, business logic, or workflows were modified. Only branding, a new public landing page, and a non-functional demo-credentials card were introduced.
  - **Product rename**: `EstateOS / Real Estate CRM` → **BuilderOne CRM** (a product by MICROTECHNIQUE IT / MITCS). The BuilderOne logo and MITCS hexagon logo replaced every visible product identifier across the public surfaces. Logos provided as JPGs with a solid black background were programmatically cleaned via Pillow (thresholded black-to-alpha) and saved as transparent PNGs at `/app/frontend/public/builderone-logo.png`, `/app/frontend/public/builderone-logo-cropped.png`, `/app/frontend/public/mitcs-logo.png`, plus `/favicon.png` + `/favicon.ico` cropped to the icon mark.
  - **New landing page** (`/`, public, prepended in front of `/login`):
    - File added: `frontend/src/pages/LandingPage.tsx` (single file, ~440 lines, no new deps — uses existing lucide-react + Button shadcn primitive).
    - Theme: **light premium SaaS** with navy primary (matching CRM tokens) and amber/gold accents (matching BuilderOne logo). Decorative grid background, blurred radial gradients, subtle micro-interactions on cards, animated WhatsApp ping.
    - 7 sections as per spec: Sticky-nav header → Hero (BuilderOne CRM + "Smart CRM for Builders & Real Estate Teams" + Explore + WhatsApp buttons + hero logo card with 3 stat tiles) → Features grid (10 cards: Lead Management, Property Management, Client Management, Deal Pipeline, WhatsApp Integration, Analytics Dashboard, Team Management, Follow-ups, Reports, Notifications) → Workflow vertical timeline (7 steps with numbered badges + connecting line) → Why BuilderOne (5 reasons w/ gradient-accented icons) → About product (dark gradient card with MITCS logo + microtechniqueit.com link) → CTA (Explore + WhatsApp) → Footer with BuilderOne CRM © 2026 + "A product by MICROTECHNIQUE IT".
    - Floating WhatsApp button (fixed bottom-right, animate-ping ring, hover tooltip) → `wa.me/916355997080?text=Hey%2C%20I%20am%20interested%20in%20BuilderOne%20CRM`. All 6 WhatsApp links across the page resolve to the same URL.
    - "Explore" button is `useAuth`-aware: routes authenticated users straight to `/dashboard`, unauthenticated to `/login`.
    - Test-IDs surface: `landing-page`, `landing-nav-logo`, `nav-explore-button`, `nav-whatsapp-link`, `hero-headline`, `hero-explore-button`, `hero-whatsapp-button`, `hero-visual`, `features-grid`, `feature-card-{slug}`, `workflow-steps`, `workflow-step-{i}`, `why-grid`, `why-card-{i}`, `mitcs-logo-block`, `mitcs-website-link`, `cta-explore-button`, `cta-whatsapp-button`, `footer-copy`, `floating-whatsapp-button`.
  - **Login page rebrand + demo card** (`pages/LoginPage.tsx`, full rewrite of the same file — pre-existing form logic preserved verbatim):
    - Logo: lucide `<Building2>` icon + "EstateOS" text → `<img src="/builderone-logo-cropped.png">`.
    - Headline: "Sign in to your CRM workspace" → "Sign in to your BuilderOne CRM workspace".
    - Right pane quote updated to BuilderOne tagline ("Smart CRM for builders & real estate teams — built to close faster.") with amber accent.
    - **Demo Credentials card** (additive, NO backend/seed change as confirmed by user): amber-gradient card above the form showing `demo@builderone.com` / `demo@builderone.com` with per-field copy buttons (clipboard API, 1.5 s check-mark feedback) and a "Use demo credentials →" shortcut that auto-fills the inputs. Test-IDs: `demo-credentials-card`, `demo-email`, `demo-password`, `copy-demo-email`, `copy-demo-password`, `use-demo-credentials`. Demo user is NOT seeded — user explicitly opted to "Display card + configure later".
    - "Back to home" link in the top-left routes back to `/` (the landing page). Test-ID: `back-to-landing`.
    - Footer updated to: `BuilderOne CRM © 2026` / `A product by MICROTECHNIQUE IT`. Test-ID: `login-footer`.
    - Login submission, error handling, password visibility toggle, and post-login `/dashboard` redirect are all unchanged.
  - **Sidebar + MobileSidebar rebrand** (`components/layout/Sidebar.tsx`, `components/layout/MobileSidebar.tsx`):
    - Replaced `<Building2 />` icon + "EstateOS" heading with `<img src="/builderone-logo-cropped.png">` (expanded) and `<img src="/favicon.png">` (collapsed). Mobile drawer header swapped similarly. All other sidebar/layout structure, nav-items, RBAC, collapse animation, tooltips, user row, logout flow — **untouched**.
  - **App.tsx routing change** (minimal): added `<Route path="/" element={<LandingPage />} />` as a public route; the protected `index` redirect-to-dashboard was removed (the landing page replaces it). Fallback wildcard now redirects to `/` instead of `/dashboard` so logged-out users land on marketing rather than the login screen. Every other route — `/dashboard`, `/leads`, `/leads/:id`, `/pipeline`, `/followups`, `/communications`, `/activity`, `/properties`, `/properties/:id`, `/clients`, `/clients/:id`, `/deals`, `/deals/board`, `/deals/:id`, `/settings`, `/users`, `/reports`, `/unauthorized` — and their `ProtectedRoute` / role guards are byte-identical to Phase 14.x.
  - **index.html + manifest.json + favicon** (`frontend/index.html`, `frontend/public/manifest.json`, `frontend/public/favicon.png|ico`):
    - Browser title → `BuilderOne CRM — Smart CRM for Builders & Real Estate Teams`.
    - Meta description / OG / Twitter tags updated to BuilderOne CRM copy.
    - `theme-color` set to navy (`#0f1d2e`) to match CRM primary.
    - Favicon swapped from default Vite `/favicon.ico` to a cropped BuilderOne icon mark (PNG + ICO multi-size).
    - PWA manifest added under `/manifest.json` with name "BuilderOne CRM", short name "BuilderOne", standalone display, navy theme.
  - **Restored Node.js backend supervisor program**: the container had again lost `/etc/supervisor/conf.d/supervisord_node_backend.conf` (same drift as Phase 2.5). Recreated it pointing at `npx tsx watch src/index.ts` on port 8002, ran `yarn install` + `prisma generate` inside `/app/backend`, and `supervisorctl reread && update`. Confirmed: SUPER_ADMIN already present (2), backend serving `/api/auth/login` against Neon successfully again, frontend → FastAPI proxy → Node backend chain healthy.
  - **Files added (3)**: `frontend/src/pages/LandingPage.tsx`, `frontend/public/manifest.json`, `/etc/supervisor/conf.d/supervisord_node_backend.conf` (restored).
  - **Files modified (5)**: `frontend/src/App.tsx`, `frontend/src/pages/LoginPage.tsx`, `frontend/src/components/layout/Sidebar.tsx`, `frontend/src/components/layout/MobileSidebar.tsx`, `frontend/index.html`. Plus the public assets folder received: `builderone-logo.png`, `builderone-logo-cropped.png`, `mitcs-logo.png`, `favicon.png`, `favicon.ico`.
  - **Files NOT touched**: every backend file (`backend/src/**`, Prisma schema, migrations, services, controllers, routes, middleware, seed scripts), `frontend/src/contexts/AuthContext.tsx`, `frontend/src/components/auth/ProtectedRoute.tsx`, every page under `frontend/src/pages/` except LoginPage, every component except the two sidebars, `frontend/src/components/layout/MainLayout.tsx`, `frontend/src/components/layout/Navbar.tsx`, `frontend/src/components/layout/NotificationPanel.tsx`, the entire `frontend/src/components/ui/` shadcn folder, all module-level components (leads, properties, clients, deals, communications, followups, activities, dashboard, users, pipeline), all service files, the tailwind config, the index.css design tokens, all data-testids on pre-existing surfaces, all RBAC, every analytics/reports/settings/whatsapp/Cloudinary integration, every existing supervisord program (frontend, backend FastAPI proxy, mongodb, nginx-code-proxy).
  - **Confirmed by visual screenshot**: Landing page hero, features, workflow, why, about (with MITCS logo block), CTA, footer, and floating WhatsApp button render correctly at 1920×800. Login page renders with the new BuilderOne logo, demo-credentials card (with copy buttons), updated headline + footer + right-pane quote. The CRM after login renders byte-identical to Phase 14.x except sidebar logo (icon mark only when collapsed, full wordmark when expanded) and mobile drawer header logo — verified against the Phase-14.3 layout.
  - **WhatsApp business number**: `+91 6355 997 080` (MITCS) → `wa.me/916355997080`. Message body: `Hey, I am interested in BuilderOne CRM`.
  - **Demo credentials** (display only, NOT seeded — user request): `demo@builderone.com` / `demo@builderone.com`. User explicitly said they will "configure it later" in Phase 16.
...
FRONTEND_URL=https://builderone-preview.preview.emergentagent.com
...
```env
REACT_APP_BACKEND_URL=https://builderone-preview.preview.emergentagent.com
VITE_BACKEND_URL=https://builderone-preview.preview.emergentagent.com
```
