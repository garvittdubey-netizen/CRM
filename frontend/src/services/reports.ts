...
export const reportsApi = {
  leads: (params?: DateRangeParams) =>
    api.get<LeadReport>('/api/reports/leads', { params }).then((r) => r.data),
  properties: () =>
    api.get<PropertyReport>('/api/reports/properties').then((r) => r.data),
  clients: () => api.get<ClientReport>('/api/reports/clients').then((r) => r.data),
  deals: (params?: DateRangeParams) =>
    api.get<DealReport>('/api/reports/deals', { params }).then((r) => r.data),
  agents: () =>
    api.get<{ data: AgentReportRow[] }>('/api/reports/agents').then((r) => r.data.data),
};
...
export function reportExportPath(
  ...
  return { url: `/api/reports/${section}/export`, params };
}
